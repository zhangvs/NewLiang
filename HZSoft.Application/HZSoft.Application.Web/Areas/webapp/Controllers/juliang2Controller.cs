...
namespace HZSoft.Application.Web.Areas.webapp.Controllers
{
    ...
    public class juliang2Controller : Controller
    {
        ...
        ///// </summary>
        ///// <returns></returns>
        //public ActionResult ListData(WhereEntity pageInfo)
        //{
        ...
        //    };
        //    var entityList = tlbll.GetPageListH5LX(pagination, pageInfo.ToJson());
        ...
        //        htmlStr += 
        //        $"<li class='phone-li' onclick=\"toInfo('{item.TelphoneID}','{host}')\">" +
        //        $"     <div class='phone-li-left'>" +
        //        $"         <span class='phone-li-span phone-li-number'>" +
        //        $"             <span class='phone-number-span phone-important'>{item.Telphone.Substring(0,3)}</span>" +
        //        $"             <span class='phone-number-span phone-prefix'>{item.Telphone.Substring(3,4)}</span>" +
        //        $"             <span class='phone-number-span phone-important'>{item.Telphone.Substring(7)}</span>" +
        //        $"             <div class='clear'></div></span>" +
        //        $"         <span class='phone-li-span phone-li-loc'>{item.City}</span>" +
        //        $"     </div>" +
        //        $"     <div class='phone-li-right'>" +
        //        $"         <span class='phone-li-span phone-li-price'>￥{jg}</span>" +
        //        $"         <span class='phone-li-span phone-li-orig-price'>￥{jg * 2}</span>" +
        //        $"     </div>" +
        //        $" </li>";
        //    }

        //    return Content(new AjaxResult { type = ResultType.success, message = "成功", resultdata = htmlStr }.ToJson());
        //}
        ...
        public ActionResult ListData(WhereEntity pageInfo)
        {
            ...
            var entityList = tlbll.GetPageListH5LX_JS(pagination, pageInfo.ToJson());
        ...
        [HttpPost]
        public ActionResult ajaxorder(OrdersEntity ordersEntity, string smsCode)
        {
            H5Response root = null;
            //1.验证手机验证码是否有效，是否过期
            var sms = smsBll.GetList("{}").Where(t => t.Tel == ordersEntity.ContactTel && t.Type == (int)SmsType.利新靓号).OrderByDescending(t => t.CreateDate).FirstOrDefault();
            if (sms != null)
            {
                if (sms == null)
                    root = new H5Response { code = false, status = false, msg = "验证码不正确" };
                if (smsCode != sms.Captcha)
                    root = new H5Response { code = false, status = false, msg = "验证码不正确" };

                //判断是否过期
                if (sms.CreateDate.AddMonths(WebSiteConfig.SMS_EXPIRE_MIN) < DateTime.Now)
                    root = new H5Response { code = false, status = false, msg = "验证码已过期，请重新发送" };
            }
            else
            {
                root = new H5Response { code = false, status = false, msg = "验证码不存在！" };
            }
            if (root != null)
            {
                ...
                try
                {
                    ...
                    if (payType == "支付宝")
                    {
                        try
                        {
                            ...
                            model.QuitUrl = "https://hllqh1.zitiaonc.com:9016/webapp/juliang2/index";// 支付中途退出返回商户网站地址
                           ...
                           TenPayV3Type.NATIVE,
                            ...
                            if (result.return_code == "SUCCESS")
                            {
                                H5PayData h5PayData = new H5PayData()
                                {
                                    ...
                                    wx_query_href = "https://shop.jnlxsm.net/webapp/juliang2/queryWx/" + ordersEntity.Id,
                                    wx_query_over = "https://shop.jnlxsm.net/webapp/juliang2/paymentFinish/" + ordersEntity.Id
                                };
...
Request.UserHostAddress, tenPayV3Info.TenPayV3Notify, TenPayV3Type.JSAPI, openId, tenPayV3Info.Key, nonceStr);
            ...
            if (result.return_code == "SUCCESS")
            {
                WFTWxModel jsApiPayData = new WFTWxModel()
                {
                    ...
                    callback_url = "https://shop.jnlxsm.net/webapp/juliang2/paymentFinish/" + ordersEntity.Id
                };
        ...
        public ActionResult productJsApi(int? id, string host)
        {
            ...
            //商品Id，用户自行定义
            var xmlDataInfoH5 = new TenPayV3UnifiedorderRequestData(WeixinConfig.AppID2, tenPayV3Info.MchId, "JSAPI购买靓号", sp_billno,
Convert.ToInt32(Convert.ToDecimal(entity.CheckPrice) * 100),
Request.UserHostAddress, tenPayV3Info.TenPayV3Notify, TenPayV3Type.JSAPI, openId, tenPayV3Info.Key, nonceStr);
            var result = TenPayV3.Unifiedorder(xmlDataInfoH5);//调用统一订单接口
            LogHelper.AddLog(result.ResultXml);//记录日志
            var package = string.Format("prepay_id={0}", result.prepay_id);
            if (result.return_code == "SUCCESS")
            {
                WFTWxModel jsApiPayData = new WFTWxModel()
                {
                    ...
                    package = package,
                    paySign = TenPayV3.GetJsPaySign(WeixinConfig.AppID2, timeStamp, nonceStr, package, WeixinConfig.Key),
                    callback_url = "https://shop.jnlxsm.net/webapp/juliang2/paymentFinish/" + id
                };
                ViewBag.WxModel = jsApiPayData;
                LogHelper.AddLog(JsonConvert.SerializeObject(jsApiPayData));//记录日志
            }
        ...
        public void AliPay()
        {
            ...
            try
            {
                ...
                // 支付中途退出返回商户网站地址
                string quit_url = "https://hllf25.zitiaonc.com:4422/webapp/juliang2/index";
