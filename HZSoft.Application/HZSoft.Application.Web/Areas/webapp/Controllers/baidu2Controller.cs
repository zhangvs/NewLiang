...
namespace HZSoft.Application.Web.Areas.webapp.Controllers
{
    ...
    /// </summary>
    public class baidu2Controller : Controller
    {
                ...
                try
                {
                    ...
                    if (payType == "支付宝")
                    {
                        try
                        {
                            ...
                            model.QuitUrl = "http://lxlh1.shahe2.top:1117/webapp/baidu2/index";// 支付中途退出返回商户网站地址
                           ...
                           TenPayV3Type.NATIVE,
                            ...
                            if (result.return_code == "SUCCESS")
                            {
                                H5PayData h5PayData = new H5PayData()
                                {
                                    ...
                                    wx_query_href = "https://shop.jnlxsm.net/webapp/baidu2/queryWx/" + ordersEntity.Id,
                                    wx_query_over = "https://shop.jnlxsm.net/webapp/baidu2/paymentFinish/" + ordersEntity.Id
                                };
...
Request.UserHostAddress, tenPayV3Info.TenPayV3Notify, TenPayV3Type.JSAPI, openId, tenPayV3Info.Key, nonceStr);
            ...
            if (result.return_code == "SUCCESS")
            {
                WFTWxModel jsApiPayData = new WFTWxModel()
                {
                    ...
                    callback_url = "https://shop.jnlxsm.net/webapp/baidu2/paymentFinish/" + ordersEntity.Id
                };
...
Request.UserHostAddress, tenPayV3Info.TenPayV3Notify, TenPayV3Type.JSAPI, openId, tenPayV3Info.Key, nonceStr);
            ...
            if (result.return_code == "SUCCESS")
            {
                WFTWxModel jsApiPayData = new WFTWxModel()
                {
                    ...
                    package = package,
                    paySign = TenPayV3.GetJsPaySign(WeixinConfig.AppID2, timeStamp, nonceStr, package, WeixinConfig.Key),
                    callback_url = "https://shop.jnlxsm.net/webapp/baidu2/paymentFinish/" + id
                };
                ViewBag.WxModel = jsApiPayData;
                LogHelper.AddLog(JsonConvert.SerializeObject(jsApiPayData));//记录日志
            }
        ...
        public void AliPay()
        {
            ...
            try
            {
                ...
                // 支付中途退出返回商户网站地址
                string quit_url = "https://hllf25.zitiaonc.com:4422/webapp/baidu2/index";
