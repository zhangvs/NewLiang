...
namespace HZSoft.Application.Web
{
    ...
    /// 描 述：登录认证（会话验证组件）
    /// </summary>
    public class HandlerWXLoginAttribute : AuthorizeAttribute
    {
        private LoginMode _customMode;
        /// <summary>默认构造</summary>
        /// <param name="Mode">认证模式</param>
        public HandlerWXLoginAttribute(LoginMode Mode)
        {
            _customMode = Mode;
        }
        /// <summary>
        ...
        public override void OnAuthorization(AuthorizationContext filterContext)
        {
            ...
            string RequestUri =!string.IsNullOrEmpty(request.Params["urlstr"])? request.Params["urlstr"]:request.FilePath;
            //登录是否过期
            if (OperatorProvider.Provider.IsOverdue())
            {
                WebHelper.WriteCookie("hzsoft_login_error", "Overdue");//登录已超时,请重新登录
                filterContext.Result = new RedirectResult("~/WeChatManage/WXLogin/Index?urlstr="+ RequestUri);
                return;
            }
            //是否已登录
